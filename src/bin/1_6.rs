...
fn main () {
    println!("{:?}", is_msg_can_not_equip_weapon(Weapon::ZelcovaSword(String::from("ZelcovaSword")), true));
    println!("{:?}", is_msg_can_not_equip_weapon(Weapon::CarrotSword(String::from("CarrotSword")), true));
    println!("{:?}", is_msg_can_not_equip_weapon(Weapon::WoodSword(String::from("WoodSword")), true));
    println!("{:?}", is_msg_can_not_equip_weapon(Weapon::DragonSpear(String::from("DragonSpear")), true));
    println!("{:?}", is_msg_can_not_equip_weapon(Weapon::KusanagiSword(String::from("KusanagiSword")), true));
...
mod tests {
    ...
    fn test_equip_sield(#[case] select_weapon: Weapon, #[case] have_sield: bool, #[case] expected: bool) {
        assert_eq!(is_msg_can_not_equip_weapon(select_weapon, have_sield), expected);
    }
    // 教訓：不要なテストケースを書かない
}
